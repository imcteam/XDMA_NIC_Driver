		...
		struct xdma_transfer *transfer)
{
	...
	//wake_up_interruptible(&transfer->my_queue);
	//printk("transfer:%p\n\n\n",transfer);
...
static void engine_rx_work(struct work_struct *work)
{
	...
	//mdelay(1000);
	iowrite32(11, bar0);
	iowrite32(11, bar0 + 0x04);
	iowrite32(11, bar0 + 0x08);
	iowrite32(11, bar0 + 0x0c);
	iowrite32(11, bar0 + 0x10);
	iowrite32(11, bar0 + 0x14);
		       ...
		       int offset, enum dma_data_direction dir, int channel)
{
	...
	INIT_WORK(&engine->work_rx, engine_rx_work);
...
ssize_t xdma_xfer_submit_net(void *dev_hndl,void *net_req)
{
	...
	rv = transfer_init(engine, req, &req->tfer[1]);
	if (rv < 0) {
